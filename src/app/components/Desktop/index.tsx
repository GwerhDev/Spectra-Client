...
const looksLikeSpectra = (app: App): boolean =>
  /spectra/i.test(app.url) || /spectra/i.test(app.label);
...
export function Desktop() {
  ...
  // Current app: prefer a host match (prod), fall back to a "spectra" match (dev).
  const currentApp = apps.find((a) => sameHost(a.url)) ?? apps.find(looksLikeSpectra);
            ...
            {apps.map((app) => {
              ...
              return (
                <button
                  ...
                  style={{ '--app-color': app.color ?? 'var(--accent)' } as React.CSSProperties}
